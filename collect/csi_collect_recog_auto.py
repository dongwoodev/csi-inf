    ...
    def recording(self, isStarted, isProcess):
        try:
            while True:
                ...
                print(len(resultA.boxes))
                # 사람의 수가 한 명 이상의 경우
                if ((len(resultA.boxes) >= 1 or len(resultB.boxes) >= 1) or currentTime in [30,31]) and (isProcess.value == False):
                    isStarted.value = True # CSI 데이터 수집 ON
                    # isHuman.value = True -> 위 코드로 옮김
                ...
                # 현재 데이터를 수집 중인 상태
                elif ((len(resultA.boxes) >= 1 or len(resultB.boxes) >= 1) or (currentTime in [30,31])) and (isProcess.value == True):
                    ...
                    # Processed 사진 데이터 저장
                    resultA.save(filename=f"{self.CheckedFolderPath}/{timestamp_dirs}/{timestamp_image}_L.jpg") # Save Processed Image File
                    resultB.save(filename=f"{self.CheckedFolderPath}/{timestamp_dirs}/{timestamp_image}_R.jpg")

                    # CSV에 사람 정보 저장
                    if len(resultA.boxes) == 1:
                        writed.writerow([f"{timestamp_dirs}/{timestamp_image}_L.jpg", len(resultA.boxes), "", ""]+ list(resultA.boxes[0].xyxy) + list(resultA.keypoints[0].xy)) # Write Row Data in CSV File
                    else:
                        writed.writerow([f"{timestamp_dirs}/{timestamp_image}_R.jpg", len(resultA.boxes), "", ""])                   

                    if len(resultB.boxes) == 1:
                        writed.writerow([f"{timestamp_dirs}/{timestamp_image}_L.jpg", len(resultB.boxes), "", ""]+ list(resultB.boxes[0].xyxy) + list(resultB.keypoints[0].xy)) # Write Row Data in CSV File
                    else:
                        writed.writerow([f"{timestamp_dirs}/{timestamp_image}_R.jpg", len(resultB.boxes), "", ""])    

                # 데이터 수집을 중단하는 상태
                elif ((len(resultA.boxes) == 0 and len(resultB.boxes) == 0) or (currentTime not in [30,31])) and (isProcess.value == True):
