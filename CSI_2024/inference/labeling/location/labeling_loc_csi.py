        ...
        try:
            for csi_file in self.csifiles:
                ...
                while current_start < end_time:
                    ...
                    location_data = self.location_df[(self.location_df['timestamp'] >= current_start) & (self.location_df['timestamp'] < current_end)]
                    if not location_data.empty:
                        location_data = location_data['location'].iloc[0]
                    else:
                    ...
                    # # pose
                    # static action
                    location = str(filtered_csi_data[:1]['location'].values[0])
                    if location == 'AP':
                        save_filename = f'./complete/AP/{current_start.strftime("%Y-%m-%d_%H-%M-%S.%f")[:-4]}.csv'
                    elif location == 'ESP':
                        save_filename = f'./complete/ESP/{current_start.strftime("%Y-%m-%d_%H-%M-%S.%f")[:-4]}.csv'
                    elif location == 'Mid':
                        save_filename = f'./complete/MID/{current_start.strftime("%Y-%m-%d_%H-%M-%S.%f")[:-4]}.csv'                    
