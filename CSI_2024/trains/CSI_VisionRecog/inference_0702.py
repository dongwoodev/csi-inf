...
def get_inference(img):
    ...
    try: 
        ...
        pose_class = 'Error'  # Handle exception case        
    return pose_class
...
# Inference Image Folder - Taehyeon 
elif args['source'].endswith('/'):
    file_list = glob.glob(args['source']+'*') 
    img_list = sorted([file for file in file_list if file.endswith(".jpg")])
    print(img_list[0])
    f = open(f"./{img_list[0][:-4]}.csv", "w")
    writer = csv.writer(f)
    datas =[['timestamp','labels']]

    for img in img_list:
        im = cv2.imread(img)
        pose_class = get_inference(im)
        row = [img[5:-7], pose_class]
        datas.append(row)
        print(img[5:-7])
        print(pose_class)
...
# python inference_0702.py -p yolov8x-pose -m ./runs/train3/model.h5 -s demo/ --conf 0.5
