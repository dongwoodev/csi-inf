...
def preprocessing(path: str):
    ...
    if not os.path.exists(f'processed_L'):
        os.mkdir(f'processed_L') # Create Directory
        os.mkdir(f'processed_L/sit')
        os.mkdir(f'processed_L/stand')
    ...
    while current_time <= end_time:
        ...
        if split_df['labels'].nunique() == 1:
            ...
            if label == "stand":        
                ...
                csi_df.to_csv(f"./processed_L/stand/{current_time}.csv", sep=',', header=False, index=False)
            else:
                csi_df = pd.read_csv(f"./{dirs}/{current_time}.csv")
                csi_df.insert(193, '', label)
                csi_df.to_csv(f"./processed_L/sit/{current_time}.csv", sep=',', header=False, index=False)                    
        current_time = next_time
...
paths = glob.glob('./*_L.csv')

# # paths = glob.glob('./*_L.csv' & './*_R.csv')
