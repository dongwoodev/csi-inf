...
class CsiLabeling:
    def __init__(self):
        self.action_df = pd.read_csv('action.csv', encoding='utf-8')
        # # pose
        # static action
    ...
    def saved_csi_label(self, grid, stride):
        ...
        self.action_df['timestamp'] = self.action_df['timestamp'].str.replace('_', ' ')
        self.action_df['timestamp'] = pd.to_datetime(self.action_df['timestamp'])
        # # pose
        # static action
        ...
        try:
            for csi_file in self.csifiles:
                ...
                start_time = self.action_df['timestamp'].min()
                end_time = self.action_df['timestamp'].max()
                ...
                while current_start < end_time:
                    ...
                    # Extract Action data
                    action_data = self.action_df[(self.action_df['timestamp'] >= current_start) & (self.action_df['timestamp'] < current_end)]
                    if not action_data.empty:
                        action_data = action_data['action'].iloc[0]
                    else:
                        # 해당 시간 내 액션 데이터가 없는 경우
                        current_start += stride
                        continue
                    filtered_csi_data = filtered_csi_data.assign(action=action_data)
                    # # pose
                    # static action
                    action = str(filtered_csi_data[:1]['action'].values[0])
                    if action == 'sit':
                        save_filename = f'./complete/sit/{current_start.strftime("%Y-%m-%d_%H-%M-%S.%f")[:-4]}.csv'
                    elif action == 'stand':
                        save_filename = f'./complete/stand/{current_start.strftime("%Y-%m-%d_%H-%M-%S.%f")[:-4]}.csv'            
...
if __name__ == '__main__':
    ...
    grid = int(args.grid) # path of action csv data files
